...
class ParseAgentOutputTests(unittest.TestCase):
    ...
    def test_parse_multiline_action_payload(self) -> None:
        kind, payload = react_agent.parse_agent_output(
            "ACTION:\npython_eval (10+20+30)/3"
        )

        self.assertEqual(kind, "action")
        self.assertEqual(payload, "python_eval (10+20+30)/3")

    def test_parse_multiline_final_payload(self) -> None:
        kind, payload = react_agent.parse_agent_output(
            "THOUGHT: concluir\nFINAL:\nResposta consolidada aqui."
        )

        self.assertEqual(kind, "final")
        self.assertEqual(payload, "Resposta consolidada aqui.")
...
class RetryAndQuotaTests(unittest.TestCase):
    def test_parse_retry_delay_from_ms_message(self) -> None:
        ...
        delay = agent_core.parse_retry_delay_seconds(error_body)
    ...
    def test_detects_quota_exceeded_error(self) -> None:
        ...
        self.assertTrue(agent_core.is_quota_exceeded_error(error_body))
